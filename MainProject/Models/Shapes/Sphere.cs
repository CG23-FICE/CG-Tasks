...
namespace MainProject.Models.Shapes
{
    public class Sphere : IBaseObject, IIntersectable
    {
        ...
        {
            var RayDirect2 = Vector.Dot(ray.Direction.ToVector(), ray.Direction.ToVector()); //d2
            var Radius2 = Radius * Radius; //r2
            var K2 = Vector.Dot(new Vector(Center, ray.Origin), new Vector(Center, ray.Origin)); //(o-c)2

            var a = RayDirect2;
            var b = 2 * Vector.Dot(ray.Direction.ToVector(), new Vector(Center, ray.Origin)); // 2*d*(o-c) 
        ...
        {
            return new Normal(Center, point).ToVector();
        }
    }
}
